...
const SIBLING_LEARN = path.resolve(import.meta.dirname, "../../learn");
const CLONE_LEARN = path.resolve(import.meta.dirname, "../.tmp-learn-mirror");
const LEARN_REPO_URL = "https://github.com/w3-kit/learn.git";
...
function resolveLearnDir(): string | null {
  ...
  try {
    ...
    console.warn(`  failed to clone learn repo: ${(err as Error).message}`);
    return null;
  }
}

const LEARN_DIR = resolveLearnDir() ?? SIBLING_LEARN;
...
const NAV_OUT = path.resolve(import.meta.dirname, "../src/entities/guide/model/docs-nav.gen.ts");
...
function discoverRecipes(): RecipeInfo[] {
  ...
  for (const dir of dirs) {
    ...
    const learnFile = fs.readdirSync(recipeDir).find((f) => f.endsWith(".learn.md"));
...
function generateGuideRegistry(guides: GuideInfo[]): string {
  ...
  for (const g of guides) {
    ...
    lines.push(`    description: ${JSON.stringify(`Learn about ${g.title.toLowerCase()}`)},`);
...
function generateRecipeRegistry(recipes: RecipeInfo[]): string {
  ...
  for (const r of recipes) {
    ...
    if (r.hasSolana) {
      lines.push(`import ${prefix}Solana from "@learn/recipes/${r.slug}/solana.tsx?raw";`);
    }
    if (r.hasLearn) {
      lines.push(`import ${prefix}Learn from "@learn/recipes/${r.slug}/${r.learnFilename}?raw";`);
    }
  }
...
function generateDocsNav(docs: DocInfo[], guides: GuideInfo[], recipes: RecipeInfo[]): string {
  const lines: string[] = [
    "// AUTO-GENERATED by scripts/generate-learn-registry.ts — do not edit manually",
    "",
  ...
  for (const [category, categoryGuides] of guidesByCategory) {
    const title = guidesByCategory.size === 1 ? "Guides" : `Guides: ${categoryToTitle(category)}`;
    lines.push("  {");
